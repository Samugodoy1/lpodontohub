...
export default function AdminApplications() {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-apple-surface">
        <div className="flex flex-col items-center gap-4">
          <div className="w-10 h-10 border-4 border-apple-ink border-t-transparent rounded-full animate-spin" />
  ...
  if (!user) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-apple-surface px-5">
        <div className="max-w-md w-full bg-white p-10 rounded-[2.5rem] shadow-xl border border-slate-100 text-center">
          <div className="w-16 h-16 bg-apple-surface text-apple-ink rounded-2xl flex items-center justify-center mx-auto mb-8">
          ...
          <Button onClick={handleLogin} className="w-full py-4 bg-[#0071e3] text-white font-bold rounded-2xl">
            Login com Google
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-apple-surface pb-20">
      <header className="bg-white border-b border-slate-100 sticky top-0 z-30">
        <div className="max-w-[1600px] mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-6">
            <a href="/academy" className="text-slate-400 hover:text-apple-ink transition-colors">
              <ChevronLeft size={24} />
            </a>
            <div className="h-8 w-px bg-slate-100" />
            <h1 className="font-semibold text-xl text-brand-text tracking-tight flex items-center gap-3">
              <Users size={20} className="text-apple-ink" />
              Inscrições Academy
            </h1>
          </div>
          <div className="flex items-center gap-4">
            <div className="hidden md:block text-right">
              ...
              <p className="text-[10px] font-bold text-slate-300 ">{user.email}</p>
            </div>
            <button 
              onClick={handleLogout}
              className="p-3 text-slate-300 hover:text-red-500 transition-colors bg-apple-surface rounded-xl"
            >
              <LogOut size={20} />
            </button>
          </div>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 mb-10">
            <div className="relative w-full md:w-96">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-4 bg-white border border-slate-200 rounded-2xl text-sm font-medium focus:ring-2 focus:ring-brand-academy focus:border-apple-ink outline-none transition-all shadow-sm"
                value={searchTerm}
          ...
          <div className="bg-white rounded-3xl border border-slate-100 shadow-xl overflow-hidden">
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse min-w-[1200px]">
                <thead>
                  <tr className="bg-apple-surface border-b border-slate-100">
                    <th className="px-6 py-5 text-[10px] font-semibold text-slate-400 ">Nome</th>
                    <th className="px-6 py-5 text-[10px] font-semibold text-slate-400 ">Social</th>
                    <th className="px-6 py-5 text-[10px] font-semibold text-slate-400 ">Acadêmico</th>
                    <th className="px-6 py-5 text-[10px] font-semibold text-slate-400 ">Atuação</th>
                    <th className="px-6 py-5 text-[10px] font-semibold text-slate-400 ">Métricas</th>
                    <th className="px-6 py-5 text-[10px] font-semibold text-slate-400  text-right">Ação</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-50">
                  {filteredApplications.length > 0 ? (
                    filteredApplications.map((app) => (
                      <tr key={app.id} className="hover:bg-apple-surface/50 transition-colors group">
                        ...
                        <td className="px-6 py-6">
                          <div className="flex flex-col gap-1">
                            <a href={`https://instagram.com/${app.instagram.replace('@', '')}`} target="_blank" rel="noreferrer" className="text-xs font-bold text-apple-ink flex items-center gap-1.5 hover:underline">
                        ...
                        <td className="px-6 py-6">
                          ...
                          <p className="text-[10px] font-semibold text-slate-400 uppercase">{app.period}</p>
                        </td>
                        <td className="px-6 py-6">
                          <p className="text-xs font-bold text-brand-text mb-0.5">{app.attendingClinic}</p>
                          <p className="text-[10px] font-semibold text-slate-400 uppercase">Período Clínico</p>
                        </td>
                        <td className="px-6 py-6">
                          <div className="flex gap-4">
                            <div className="text-center">
                              <p className="text-xs font-bold text-brand-text">{app.followers}</p>
                              <p className="text-[9px] font-semibold text-slate-300 uppercase">Segs</p>
                            </div>
                            <div className="text-center">
                              <p className="text-xs font-bold text-brand-text">{app.storyViews}</p>
                              <p className="text-[9px] font-semibold text-slate-300 uppercase">Views</p>
                            </div>
                            <div className="text-center">
                              <p className="text-xs font-bold text-brand-text">{app.postsPerWeek}</p>
                              <p className="text-[9px] font-semibold text-slate-300 uppercase">Posts/Sem</p>
                            </div>
                          </div>
                        </td>
                        <td className="px-6 py-6 text-right">
                           <button 
                            className="p-2.5 text-slate-200 hover:text-apple-ink hover:bg-white rounded-xl transition-all shadow-none hover:shadow-sm"
                            title="Ver Detalhes"
                           >
                    ...
                    <tr>
                      <td colSpan={6} className="px-6 py-20 text-center">
                        <p className="text-sm font-bold text-slate-300 ">Nenhuma inscrição encontrada</p>
                      </td>
                    </tr>
                  )}
                </tbody>
